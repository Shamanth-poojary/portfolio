...
export default function EditProject({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (fetching) return <div className="text-white p-24 text-center">Loading Project...</div>;
    ...
    <div className="min-h-screen bg-surface-1 py-24 px-8 flex justify-center text-text-main">
      <div className="max-w-2xl w-full bg-white/[0.03] backdrop-blur-2xl border border-white/10 p-8 rounded-[2rem] shadow-2xl">
        <h1 className="text-3xl font-display font-extrabold tracking-tight mb-8">
          Edit <span className="text-brand">Project</span>
        </h1>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-sm text-text-muted font-medium">Update Preview Image (Leave blank to keep existing)</label>
            <input 
              type="file" 
              accept="image/*"
          ...
          >
            {loading ? "Updating..." : "Update Database Entry"}
          </button>
        </form>
      </div>
    </div>
